...
import { updateSeason } from '../../services/seasonService';
import { useNotification } from '../../context/NotificationContext';
import Button from '../common/Button';
import { Card } from '../common/Card';

export default function EditSeasonModal({ season, games, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (season) {
      setFormData({
        ...
        status: season.status || 'PREPARING',
      });
    }
  }, [season]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await updateSeason(season.id, formData);
      ...
      showError(error?.message || 'Cập nhật mùa giải thất bại');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <Card className="max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-primary-700/20">
          <div>
            <h2 className="text-2xl font-bold text-white">Chỉnh sửa Mùa giải</h2>
            <p className="text-sm text-gray-400 mt-1">Cập nhật thông tin mùa giải #{season.id}</p>
          </div>
        </div>
